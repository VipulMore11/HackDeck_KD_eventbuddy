...
@api_view(['GET'])
@permission_classes([IsAuthenticated])
def get_start_end_date(request):
    try:
        user = request.user

        # Query events where the user is the organiser
        organiser_events = Event.objects.filter(organiser__user=user).values('title', 'start_time', 'end_time')

        # Query tasks where the user is assigned as a staff member and get associated events
        staff_tasks = Tasks.objects.filter(assigned_to__user=user).select_related('event')
        staff_events = staff_tasks.values('event__title', 'event__start_time', 'event__end_time')

        # Combine both organiser and staff events into a unified response
        events = list(organiser_events) + [
            {
                'title': event['event__title'],
                'start_time': event['event__start_time'],
                'end_time': event['event__end_time']
            }
